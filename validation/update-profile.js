...
module.exports = function validateProfileInput(userData) {
  ...
  if (
    !validator.isEmpty(userData.university) &&
    !validator.isLength(userData.university, { min: 1, max: 40 })
  ) {
    errors.university = 'University needs to be between 1 and 100 characters';
  }
  // Adds an error if the bio is not the correct length
  if (
    !validator.isEmpty(userData.bio) &&
    !validator.isLength(userData.bio, { min: 1, max: 300 })
  ) {
  ...
  if (
    !validator.isEmpty(userData.handle) &&
    !validator.isAscii(userData.handle)
  ) {
